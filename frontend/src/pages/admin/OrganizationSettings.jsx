    ...
    return (state.organizations || [])
      .filter((org) => {
        ...
        return (
          ...
          (org.contactEmail || '').toLowerCase().includes(normalized)
        );
      })
      .map((org) => {
        const locationsCount = (state.locations || []).filter((location) => location.organizationId === org.id).length;
        return {
          ...org,
        ...
        <div className="p-4 border-b border-slate-200 dark:border-slate-800 bg-slate-50/80 dark:bg-slate-900/80 backdrop-blur-md flex justify-between items-center z-10 sticky top-0">
          ...
          <div className="relative w-64">
            ...
            <input
              ...
              className="w-full bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-700 rounded-full pl-9 pr-4 py-2 text-sm outline-none focus:ring-2 focus:ring-[var(--color-brand-terracotta)] dark:focus:ring-red-500 dark:text-slate-200 font-medium"
            />
          </div>
        </div>
                  ...
                  {org.expanded && (
                    ...
                    >
                      <div className="p-5 pl-17">
                        ...
                        {state.locations.filter((location) => location.organizationId === org.id).length > 0 ? (
                          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                            {state.locations
                              .filter((location) => location.organizationId === org.id)
                              .map((location) => {
                                const mapNodeCount = state.maps?.[location.id]?.nodes?.length || 0;
                                const activeVisitorCount = (state.visitors || []).filter(
                                  (visitor) => visitor.status === 'active' && visitor.locationId === location.id,
                                ).length;

                                return (
                                  <div key={location.id} onClick={() => navigate(`/staff/map-editor?locationId=${encodeURIComponent(location.id)}`)} className="p-4 bg-white dark:bg-slate-800 border-l-4 border-l-[var(--color-brand-terracotta)] dark:border-l-red-500 rounded-xl rounded-l-md shadow-sm border border-slate-200 dark:border-slate-700 relative group cursor-pointer hover:border-[var(--color-brand-terracotta)] dark:hover:border-red-500 transition-colors">
                                <MapPin size={24} className="text-slate-300 dark:text-slate-600 mb-2" />
                                <h5 className="font-bold text-slate-900 dark:text-white">{location.name}</h5>
                                <p className="text-xs text-slate-500 mt-1 mb-3">{mapNodeCount} Map Nodes • {activeVisitorCount} Active Visitors</p>
          ...
          <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
            ...
            >
              ...
              <div className="p-6 space-y-4">
                ...
                <div className="space-y-1">
                  <label className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest pl-1">Receptionist Email</label>
                  <input type="email" defaultValue={editingOrg ? editingOrg.email : ''} placeholder="reception@acme.com" className="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-700 rounded-lg px-3 py-2.5 text-sm dark:text-white outline-none focus:ring-2 focus:ring-[var(--color-brand-terracotta)]" />
                </div>
                <div className="space-y-1">
            ...
            >
              ...
              <div className="p-6 space-y-4">
                <div className="space-y-1">
                  ...
                  <input type="text" value={locationName} onChange={(e) => setLocationName(e.target.value)} placeholder="e.g. Warehouse A" className="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-700 rounded-lg px-3 py-2.5 text-sm dark:text-white outline-none focus:ring-2 focus:ring-[var(--color-brand-terracotta)]" />
                </div>
                <div className="space-y-1">
